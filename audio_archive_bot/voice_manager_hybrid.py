...
class HybridVoiceManager:
    ...
    async def start_recording(guild_id: int, channel, target_user_id: int) -> bool:
        ...
        try:
            logger.info(
                f"🎙️ Starting hybrid voice recording - Guild: {guild_id}, Channel: {channel.name}"
            )
            ...
            if success:
                logger.info("✅ Recording command sent to Node.js component")
                return True
            else:
    ...
    async def stop_recording(guild_id: int) -> bool:
        ...
        try:
            ...
            success = HybridVoiceManager.send_command("stop_recording", guildId=str(guild_id))

            if success:
                logger.info("✅ Stop command sent to Node.js component")
                return True
            else:
