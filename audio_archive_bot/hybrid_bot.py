...
if BACKGROUND_MODE:
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("hybrid_bot.log"), logging.StreamHandler()],
    )
...
async def background_maintenance():
    ...
    while True:
        try:
            ...
            old_sessions = [
                k
                for k, v in recording_sessions.items()
                if hasattr(v, "start_time")
                and (current_time - v.start_time).total_seconds() > 86400
            ]
...
async def on_ready():
    ...
    if status:
        ...
        logger.warning(
            "⚠️ No status from Node.js voice recorder - ensure voice_recorder.js is running"
        )
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        logger.info(
            f"🎯 TARGET USER JOINED: {username} in {after.channel.name} (Guild: {member.guild.name})"
        )

        # Start recording via Node.js
        success = await HybridVoiceManager.start_recording(
            guild_id=guild_id, channel=after.channel, target_user_id=TARGET_USER_ID
        )
    ...
    # User left voice channel
        ...
        logger.info(
            f"🎯 TARGET USER LEFT: {username} from {before.channel.name} (Guild: {member.guild.name})"
        )
    ...
    # User switched channels
        ...
        before.channel is not None and after.channel is not None and before.channel != after.channel
    ):
        logger.info(
            f"🔄 TARGET USER MOVED: {username} from {before.channel.name} to {after.channel.name}"
        )
...
async def test_recording(ctx):
    ...
    if ctx.author.voice and ctx.author.voice.channel:
        ...
        success = await HybridVoiceManager.start_recording(
            guild_id=ctx.guild.id, channel=channel, target_user_id=ctx.author.id
        )
